...
class News {
    draw(data: Array<INew>): void {
        ...
        news.forEach((item: INew, idx: number) => {
            ...
            if (newsCloneSelector) newsCloneSelector.textContent = item.author || item.source.name;
            ...
            if (newsCloneSelector) newsCloneSelector.textContent = item.title;

            newsCloneSelector = newsClone.querySelector('.news__description-source');

            if (newsCloneSelector) newsCloneSelector.textContent = item.source.name;

            newsCloneSelector = newsClone.querySelector('.news__description-content');

            if (newsCloneSelector) newsCloneSelector.textContent = item.description;

            newsCloneSelector = newsClone.querySelector('.news__read-more a');

            if (newsCloneSelector) newsCloneSelector.setAttribute('href', item.url);
        ...
        if (querySelector) querySelector.innerHTML = '';

        querySelector = document.querySelector('.news');

        if (querySelector) querySelector.appendChild(fragment);
    }
}
